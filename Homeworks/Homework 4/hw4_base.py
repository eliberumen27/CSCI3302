...
#color_ranges = [((100, 0, 0), (255, 0, 0)), ((0,100,0), (0,255,0)), ((0,0,100), (0,0,255))]
color_ranges = []
...
def do_color_filtering(img):
  ...
  #mask2 = np.zeros([img_height, img_width])
  ...
  for ypixel in range(img_height):
      for xpixel in range(img_width):
          if check_if_color_in_range((ypixel, xpixel)):
              mask[ypixel, xpixel] = 1

  return mask # Or mask 2?
...
def expand_nr(img_mask, cur_coord, coordinates_in_blob):
  ...
  while len(coordinate_list) > 0:
    ...
    # EXCEPT DO THIS NON RECURSIVELY ***
    above = [cur_coordinate[0]-1, cur_coordinate[1]]
    below = [cur_coordinate[0]+1, cur_coordinate[1]]
    left = [cur_coordinate[0], cur_coordinate[1]-1]
    right = [cur_coordinate[0], cur_coordinate[1]+1]
    for coord in [above, below, left, right]:
      expand_nr(img_mask, coord, coordinates_in_blob)
...
def get_blobs(img_mask):
  ...
  #blob_coords = [] this is the list defined above already
  for ypixel in range(img_mask_height):
      for xpixel in range(img_mask_width):
          if newMask[ypixel, xpixel] == 1:
              currList = []
              expand(newMask, (ypixel, xpixel), currList)
              blobs_list.append(currList) # or add (xpixel, ypixel) ?
...
def get_blob_centroids(blobs_list):
  ...
  # In the following we need to add the tuple formatted centroids of all the lists into object_positions_list
  for list in blobs_list: # We need the mean x and y for each list
      #currMean = np.mean(list, axis=0)
      #M = cv2.moments(list)
      # calculate x,y coordinate of center (mean of all x and mean of all y)
      xVals = []
      yVals = []
      for entry in list:
      ...
      xMean = np.mean(xVals)
      yMean = np.mean(yVals)
      object_positions_list.append((xMean, yMean))



      #object_positions_list.append((cX, cY))
